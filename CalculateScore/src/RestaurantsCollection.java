...
public class RestaurantsCollection {
	...
	private ArrayList<ZipCodeEntry> restList;
	private int minSize;
	private int maxSize;
	...
	public RestaurantsCollection() {
		restList = new ArrayList<ZipCodeEntry>();
		minSize = Integer.MAX_VALUE;
		maxSize = Integer.MIN_VALUE;
		intervals = new ArrayList<Integer>();
	}

	public ArrayList<ZipCodeEntry> getRestList() {
		return restList;
	}
		...
		try {
			...
			if (zipcodeFile.isFile() && zipcodeFile.exists()) {
				...
				while ((lineTxt = bufferedReader.readLine()) != null) {
					restList.add(new ZipCodeEntry(lineTxt));
				}
				read.close();
			} else {
